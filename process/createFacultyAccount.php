...
$fullName = $_SESSION['id'];
    ...
    $sql = "INSERT INTO accounts (id, first_name, middle_name, last_name, username, password, advisory_class, profile_picture, account_type, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters
        $stmt->bind_param('ssssssssss', $facultyId, $firstName, $middleName, $lastName, $username, $hashedPassword, $advisoryClass, $profilePicture, $accountType, $createdAt);
