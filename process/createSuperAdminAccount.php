...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id = isset($_POST['superAdminID']) ? $_POST['superAdminID'] : '';
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
             ...
             if ($logStmt = $conn->prepare($logSql)) {
                 $actionPerformed = "Created superadmin account";
                 $loggedDate = date('Y-m-d H:i:s'); // Capture current date and time
                 $logStmt->bind_param('sss', $actionPerformed, $fullName, $loggedDate);
                 $logStmt->execute();
                 $logStmt->close();
             }
            echo json_encode(['success' => 'Superadmin account created successfully.']);
        } else {
            echo json_encode(['error' => 'Failed to create superadmin account.']);
        }
